...
| 2026-06-08 | **Full-screen immersive (edge-to-edge).** `MainActivity` hides BOTH system bars (`WindowInsetsControllerCompat.hide(systemBars())`, sticky `BEHAVIOR_SHOW_TRANSIENT_BARS_BY_SWIPE`), re-applied in `onWindowFocusChanged`; RN `StatusBar hidden`. | User wanted the app to use the entire screen (no top clock/battery bar, no bottom nav bar). Sticky immersive keeps a swipe-to-reveal escape hatch. |
| 2026-06-08 | **Volume + Speed sliders moved from the top bar into the Settings (gear) modal.** | User request; declutters the top bar. Top bar reflows with a flex spacer. |
| 2026-06-08 | **Added `react-native-reanimated@4 + react-native-worklets + react-native-gesture-handler@3`** (deviates from the "no extra native deps / pinned stack" stance — recorded here). Babel `react-native-worklets/plugin` (last); `GestureHandlerRootView` at app root AND inside `SettingsModal` (RN Modal is a separate Android window). | PanResponder+`Animated` sliders still stuttered on continuous drag (PanResponder events are JS-thread). Reanimated runs the whole gesture as UI-thread worklets. CLAUDE.md §1 already anticipates Reanimated ("drive key glow with Reanimated shared values"), so it's architecturally sanctioned. Same foundation can later smooth keyboard glow. |
| 2026-06-08 | **Slider = worklet `Gesture.Pan` + shared-value position.** Thumb/fill via `useAnimatedStyle` (transform translateX/scaleX, not layout `left/width`). Live `onChange` (engine gain) + `onCommit`-on-release (persist) via `runOnJS`. Live numeric label = read-only animated `TextInput` (`useAnimatedProps`) bound to the SAME shared value. | Zero React re-renders / zero JS-thread work during a drag → fluid on low-end; the value text tracks the thumb in real time without a per-frame setState. |
| 2026-06-10 | **Top UI reworked to match `ref/top ui.png` (two captioned-card rows).** Row 1: REC · LIST · InstrumentSelector (moved inline) · spacer · SongControl · gear — dropped the "Piano · Phase 5" brand. Row 2: RECORD KEYS · SEQUENCE (new ‹ › arrows nudge the window ±4 white keys via `setWindowStart`) · KEY SIZE · SHOW NOTES (moved here). `RecordKeysControls`/`ZoomControls`/`NotationToggle` restyled as caption-on-top cards. | User-requested visual match to the reference. Arrows reuse the existing window/scroll store (no new state). | 
| 2026-06-10 | **Settings "Reset to defaults" button.** Defaults extracted to a single `DEFAULTS` const in `settingsStore` (used by both initial state + `resetToDefaults`, which re-applies `setMasterGain`/`setReleaseTime`). Modal body wrapped in a **`ScrollView` from `react-native-gesture-handler`** (NOT RN's) so it scrolls on short landscape screens; sheet gets `maxHeight: '92%'`. | User wanted a reset affordance + confirmed settings persist. RN's `ScrollView` would not scroll nested inside `GestureHandlerRootView` alongside the slider Pan gestures — the RNGH `ScrollView` coordinates touches correctly. Single `DEFAULTS` source prevents init/reset drift. |
| 2026-06-09 | **Ring-out ("Ring time") = native amp-envelope release, not a JS-delayed noteOff.** New JSI `setReleaseTime(sec)` (spec→`NativeAudioEngine`→`AudioEngine`→`SynthEngine`); on each NoteOff the audio thread overrides every matching voice's `ampenv.parameters.release` (a per-voice COPY → RT-safe) before `tsf_channel_note_off`. `performer.ts` now sends `noteOff` immediately; `settingsStore` pushes release = `ringSec` when Sustain on, else `OFF_RELEASE_SEC` 0.06s (quick stop), on change + rehydrate. | The old JS approach (delay `noteOff` by `ringSec`) had **no audible effect on decaying instruments** (piano): the voice decays to ~silence while held and the SF2's own release tail dominated, so Ring time appeared to do nothing. Controlling the synth's release makes Ring time govern the fade-out for **every** instrument. Keeps the audio callback alloc/lock/JNI/log-free (CLAUDE.md §2). |
...
- **2026-06-08 · Session 9 (UX: immersive + sliders).**
  - **Full-screen immersive:** `MainActivity.hideSystemBars()` hides top status bar + bottom nav bar (sticky, swipe-to-reveal), re-applied on focus; `StatusBar hidden` in `App.tsx`/`PlayerScreen`. App now uses the whole screen (edge-to-edge, `setDecorFitsSystemWindows(false)`).
  - **Moved Volume + Speed** out of the top bar into the **Settings (gear) modal**; top bar reflows via a flex spacer.
  - **Slider perf:** original PanResponder+`Animated` slider stuttered on continuous drag (JS-thread events). Added **Reanimated 4 + Worklets + Gesture Handler 3** (see Decisions Log) and rewrote `Slider` as a UI-thread worklet `Gesture.Pan` driving a shared value (transform translateX/scaleX). `onChange` (live engine gain) + `onCommit` (persist on release) via `runOnJS`. New `SliderValueLabel` (animated read-only `TextInput` via `useAnimatedProps`) shows the value **live** during drag, bound to the same shared value — no per-frame re-render.
  - Setup: babel `react-native-worklets/plugin`; `import 'react-native-gesture-handler'` first in `index.js`; `GestureHandlerRootView` at app root + inside `SettingsModal` (Modal is a separate Android window). Jest: lightweight manual mocks for reanimated/gesture-handler + `transformIgnorePatterns` allowlist.
  - Verified on emulator-5554: native rebuild **BUILD SUCCESSFUL** (`libreanimated.so`/`libworklets.so` load), app runs no JS errors, drag is smooth and the % / × labels update in real time. `tsc` clean, `jest` 1/1. (Audio still silent on this emulator — host-audio bridge, not code; engine confirmed healthy.)
  - **Also added** `.gitattributes` (LF normalization, binary markers) and extended `.gitignore` (`*.apk`/`*.aab`/`probe.m4a`/`*.wav`).
  - **Next:** real-device pass (audio + immersive + drag feel). Phase 6 release items still human/console-driven (see RELEASE.md).
